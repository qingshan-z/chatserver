    ...
    for (Group &group : groupVec)
    {
        ...
        if (res != nullptr)
        {
            ...
            while ((row = mysql_fetch_row(res)) != nullptr)
            {
                ...
                //std::cout<<group.getUsers().size()<<endl;
                //std::cout<<user.getId()<<"#"<<user.getName()<<"#"<<user.getState()<<"#"<<endl;
            }
            mysql_free_result(res);
        }
    }
...
// {
//     /*
//     1.先根据userid查询出所属群组id，再联合allgroup表查询出群组信息
//     2.再根据群组信息，查询属于该群组的所有用户的userid，再联合user表进行多表查询，查询出用户信息
//     */
//     char sql[1024] = {0};
